...
function App() {
  ...
  return (
    <motion.div
      ...
      className="relative min-h-screen overflow-x-clip bg-[#020617] px-6 py-8 text-slate-100 md:px-12 lg:px-24"
    >
      {/* SYSTEM ONLINE Indicator: absolute positioned in right most upper most corner */}
      <div className="absolute right-6 top-6 z-50 md:right-12 md:top-8">
        <div className="inline-flex items-center gap-2 rounded-full border border-blue-500/30 bg-blue-500/10 px-3 py-1 text-xs font-semibold tracking-wide text-blue-300 backdrop-blur-md shadow-[0_0_20px_rgba(59,130,246,0.15)]">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col">
        <motion.header
          ...
          className="mb-8 flex flex-col items-center text-center font-['Outfit']"
        >
          <h1 className="text-4xl font-bold tracking-tight text-white md:text-6xl lg:text-[4.5rem] leading-tight">
            Repo<span className="text-gradient">Atlas</span>
          </h1>
          <p className="mt-4 max-w-2xl text-base leading-relaxed text-slate-400 md:text-lg font-light">
        ...
        <div className="flex w-full flex-col gap-6 lg:flex-row lg:items-start lg:gap-8">
          ...
          <div className="flex w-full flex-1 flex-col space-y-6">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
              {/* File Upload Section: Smaller & Compact */}
              <motion.section
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
              ...
              >
                ...
                <div
                  ...
                  className={`relative flex flex-col items-center justify-center overflow-hidden rounded-2xl border-2 border-dashed px-4 py-5 text-center transition-all duration-300 ${
                    isDragging
                ...
                >
                  ...
                  <div className="rounded-full bg-slate-900/80 p-2.5 mb-3 shadow-inner ring-1 ring-white/10">
                  ...
                  <p className="text-xs font-semibold text-slate-200">Drag & Drop file</p>
                  <p className="mt-1 text-[10px] text-slate-400 font-['Space_Grotesk']">.py, .ts, .go, .js</p>

                  <div className="relative mt-4 flex w-full flex-col gap-2 sm:flex-row sm:items-center sm:justify-center">
                    <label className="group relative cursor-pointer overflow-hidden rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-xs font-semibold text-slate-200 transition-all hover:bg-white/10">
                    ...
                    <motion.button
                      whileHover={{ scale: !selectedFile || uploading ? 1 : 1.03 }}
                      whileTap={{ scale: !selectedFile || uploading ? 1 : 0.97 }}
                      onClick={handleUpload}
                      disabled={!selectedFile || uploading}
                      className="relative overflow-hidden rounded-lg bg-white text-slate-950 px-5 py-2 text-xs font-bold shadow-md transition-all disabled:opacity-50 disabled:shadow-none"
                    >
                  ...
                  {selectedFile && (
                    <motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} className="mt-3 flex items-center justify-center gap-1.5 rounded-lg bg-blue-500/10 px-3 py-1 border border-blue-500/20 backdrop-blur-sm">
                  ...
                  {uploadStatus && (
                    <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className={`mt-3 text-xs font-medium ${uploadStatus.toLowerCase().includes("success") ? "text-emerald-400" : "text-rose-400"}`}>
                      {uploadStatus}
                    </motion.p>
                  )}
                </div>
              ...
              {/* GitHub Repo Section: Smaller & Compact */}
              <motion.section
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
              ...
              >
                ...
                <div className="flex flex-col gap-3">
                  <p className="text-xs leading-relaxed text-slate-400">Ingest a public GitHub repository for comprehensive codebase indexing.</p>
                  
                  <div className="relative group">
                    <div className="absolute -inset-0.5 rounded-lg bg-gradient-to-r from-blue-600 to-sky-500 opacity-20 blur-sm transition duration-500 group-focus-within:opacity-50"></div>
                    <input
                      value={repoUrl}
                      onChange={(event) => setRepoUrl(event.target.value)}
                      placeholder="GitHub Repository URL"
                      className="relative w-full rounded-lg border border-white/10 bg-[#020617]/80 px-4 py-2.5 text-xs text-slate-100 shadow-inner outline-none transition placeholder:text-slate-500 focus:border-blue-500/50 focus:bg-[#020617]"
                    />
                  </div>
                  ...
                  <motion.button
                    ...
                    className="flex w-full items-center justify-center gap-1.5 rounded-lg bg-gradient-to-r from-blue-500 to-sky-600 px-4 py-2.5 text-xs font-bold tracking-wide text-white shadow shadow-blue-500/10 disabled:opacity-50"
                  >
                    {indexingRepo ? "Indexing..." : "Index Repository"}
                  </motion.button>

                  {repoStatus && (
                    <motion.div initial={{ opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }} className={`rounded-lg border border-white/5 bg-black/20 p-2 text-center text-xs font-medium ${repoStatus.toLowerCase().includes("success") ? "text-emerald-400" : "text-rose-400"}`}>
                      {repoStatus}
                    </motion.div>
                  )}
                </div>
            ...
            <motion.div
              ...
              className="mt-4 sticky top-4 z-50 transition-all duration-300"
            >
              <QueryInput
                query={query}
                setQuery={setQuery}
          ...
          {/* Sidebar (Query History) on the right: Matches the height of compact main cards */}
          <motion.aside
            initial={{ x: 20, opacity: 0 }}
            animate={{ x: 0, opacity: 1 }}
            transition={{ delay: 0.3, duration: 0.5 }}
            className="glass-panel flex w-full flex-col rounded-3xl p-5 lg:sticky lg:top-4 lg:w-80 xl:w-96"
          >
            <div className="mb-4 flex items-center gap-3">
              <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-500/20 text-blue-400 border border-blue-500/30">
            ...
            {queryHistory.length === 0 ? (
              <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-slate-700/50 bg-slate-800/10 py-10 text-center">
                 <p className="text-xs font-medium text-slate-400">No recent queries</p>
                 <p className="text-[10px] text-slate-500 mt-1">Start asking to build history.</p>
              </div>
            ) : (
                ...
                {queryHistory.map((item, index) => {
                  ...
                  return (
                    <motion.button
                      ...
                      className={`group relative w-full overflow-hidden rounded-2xl p-3 text-left transition-all ${
                        isActive
                          ? "border border-blue-500/50 bg-blue-500/15 shadow-[0_4px_20px_rgba(59,130,246,0.1)]"
                          : "border border-slate-700/30 bg-slate-800/20 hover:border-slate-600/50 hover:bg-slate-800/40"
                      }`}
                    >
                      {isActive && (
                         <div className="absolute left-0 top-0 h-full w-[3px] bg-gradient-to-b from-sky-400 to-blue-600 rounded-l-2xl" />
                      )}
                      <p className={`truncate text-xs font-semibold transition-colors ${isActive ? 'text-white' : 'text-slate-300 group-hover:text-blue-300'}`}>
                        {item.query}
                      </p>
                      <p className="mt-1 line-clamp-2 text-[10px] leading-relaxed text-slate-500 font-['Space_Grotesk']">
                        {item.answer}
                      </p>
                    </motion.button>
                  );
